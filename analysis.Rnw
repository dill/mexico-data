...
Before performing any actual analysis, the data must be segmented. This consists of chopping up the transects and attributing counts to each of the segments.
...
#spat.mod<-dsm.fit(hn.model$ddf, response="group", formula=count~s(latitude,longitude),obsdata,segdata)
@
