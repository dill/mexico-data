...
par(mfrow=c(1,2))
plot(hr.model, showpoints=FALSE, pl.den=0, lwd=2)
ddf.gof(hr.model$ddf)
```
...
Running the model:
```{r}
mod1 <- dsm(count~s(x,y), hr.model$ddf, segdata, obsdata, method="REML")
```
...
```{r visgam1, fig.cap=""}
vis.gam(mod1, plot.type="contour", view=c("x","y"), asp=1, type="response", contour.col="black", n.grid=100)
```

`r fig_nums("visgam1")`

The `type="response"` argument ensures that the plot is on the scale of abundance.
...
```{r tweedie-fit}
mod3 <- dsm(count~s(x,y), hr.model, segdata, obsdata, family=tw(), method="REML")
summary(mod3)
```
...
mod4<-dsm(count~s(x, y, bs="so", k=15, xt=list(bnd=list(survey.area))) +
            s(depth),
...
mod6<-dsm(count~s(x,y), hr.model$ddf, segdata, obsdata,engine="gamm",correlation=corAR1(form=~sg.id|tr.id), method="REML")
```
...
Note that as well as the plots, `gam.check` also produces information about the model fitting. Of particular interest to us is the last few lines that tell us about the basis size, see [Smoother complexity], below.
...
Note the `NA` value for the GAMM (`mod6`), this is because the deviance is not reported for these models.
...
p <- ggplot() + grid_plot_obj(mod1.pred, "Abundance", pred.polys) + coord_equal() +gg.opts
...
p <- ggplot() + grid_plot_obj(mod2.pred, "Abundance", pred.polys) + coord_equal() +gg.opts
p <- p + geom_path(aes(x=x, y=y), data=survey.area)
p <- p + labs(fill="Abundance")
print(p)
```
...
```{r echo=FALSE}
fig_nums(name="mod1varplot", caption="Plot of the coefficient of variation for the study area with transect lines and observations overlaid. Note the increase in CV away from the transect lines.", display=FALSE)
```
