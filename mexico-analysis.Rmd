...
Create the offset -- the area of each cell.
```{r}
...
We can use the approach of Williams et al (2011), which accounts for uncertainty in the effective strip width by including its derivative in as a random effect in the model. This method is known as "variance propogation".
...
``` {r, fig.cap="Plot of the coefficient of variation for the study area."}
...
``` {r}
mod2 <- dsm(N~s(x,y)+s(depth), hr.model, segdata, obsdata, select=TRUE)
summary(mod2)
```
Setting `select=TRUE` here imposes extra shrinkage terms on each smooth in the model (allowing smooth terms to be removed from the model during fitting, see `?gam` for more information). Although this is not particularly useful here, this can be a good way to cut down on covariates when many are present.
...
summary(mod3)
```
Note that we have to full specify the `family` just as in `mgcv`.

However looking at a plot of the square root of the absolute value of the residuals versus the fitted values gives the following plot:
```{r tweedie-12-residplot, fig.cap="Plot of absolute value of the residuals versus the fitted values for the Tweedie model when `p=1.2`"}
plot(sqrt(abs(residuals(mod3))),predict(mod3),xlab="Square root of the absolute value of the residuals", ylab="Fitted values")
```
where as looking at if we get `p=1.7` we obtain:
``` {r tweedie-17}
mod3 <- dsm(N~s(x,y), hr.model, segdata, obsdata,
...
```{r tweedie-17-residplot, fig.cap="Plot of absolute value of the residuals versus the fitted values for the Tweedie model when `p=1.7`, note is is much flatter than the previous plot."}
plot(sqrt(abs(residuals(mod3))),predict(mod3),xlab="Square root of the absolute value of the residuals", ylab="Fitted values")
```
Note also the improvement in GCV score.

(A good value can be found by simply plotting the above along with the GCV score for the model for values of `p` between 1.1 and 1.9 and looking for the best GCV and "flattest" plot.)

***Soap film smoothing***

To account for a complex region (i.e. a region that includes peninsulae) we can use the soap film smoother. This is provided by the soap film smoother (see references, below). First, loading the package:
...
We fit the model as above, simply replacing the `ddf` model object and changing the response to be `Nhat` so the abundances are estimated per segment:
``` {r cache=TRUE}
