...
Comparing predictions from the model that included a smooth of depth, predictions are shown in Figure 10 (figure created with the code below).
...
And run checks on the GAM part of the object (shown in Figure 12):

```{r mod1-gamm-check, fig.cap="Diagnostic plots for `mod1.gamm`.", fig.width=6, fig.height=6}
gam.check(mod1.gamm$gam)
```

We can now make predictions and compare the abundance map produced by this model to the previous models (Figure 13):
