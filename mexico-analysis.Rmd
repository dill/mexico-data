...
Create the offset -- the area of each cell (the link function (log) is applied inside `dsm.predict`).
```{r}
...
``` {r}
mod1.pred <- dsm.predict(mod1, preddata, off.set)
```
...
mod4.pred<-dsm.predict(mod4,preddata, off.set)
...
mod5.pred<-dsm.predict(mod5,preddata, off.set)
