...
``` {r}
mod2<-dsm.fit(hn.model$ddf, response="indiv", formula=~s(x,y)+s(depth), obsdata=obsdata,segdata=segdata)
summary(mod2)
```
...
``` {r}
mod3<-dsm.fit(hn.model$ddf, response="indiv", formula=~s(x,y), 
		obsdata=obsdata,segdata=segdata,
...
``` {r cache=TRUE}
mod4<-dsm.fit(hn.model$ddf, response="indiv", 
