...
Also note that since we've projected our prediction grid, the "squares" don't look quite like squares. So for plotting we'll use the polygons that we've saved. This makes plotting take a little longer, but avoids gaps and overplotting. Figure 2 compares using latitude/longitude with a projection.

```{r projection-compare, fig.cap="Comparison between unprojected latitude and longitude (left) and the prediction grid projected using the North American Lambert Conformal Conic projection (right)"}
par(mfrow=c(1,2))
...
We can then make the predictions over the the grid and calculate abundance. The offset is stored in the `area` column[^offsetcorrection].
...
mod1.gamm<-dsm(N~s(x,y), hr.model$ddf, segdata, obsdata,engine="gamm",correlation=corAR1(form=~sg.id|tr.id), method="REML")
```
...
```{r mod1-gamm-check, fig.cap="Diagnostic plots for `mod1.gamm`.", fig.width=6, fig.height=6}
...
mod1.gamm.pred <- predict(mod1.gamm, preddata, preddata$area)
p <- ggplot() + grid_plot_obj(mod1.gamm.pred, "Abundance") + coord_equal() +gg.opts
p <- p + geom_path(aes(x=x, y=y), data=survey.area)
p <- p + labs(fill="Abundance")
print(p)
```
...
```{r cache=TRUE}
mod1.gamm.var <- dsm.var.prop(mod1.gamm, pred.data=preddata, off.set=preddata$area)
