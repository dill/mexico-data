...
# make the data.frame
obsdata <- data.frame(object       = 1:nrow(obs),
                      Sample.Label = obs[,13],
...
proj4string(msegsp) <- CRS("+proj=longlat +datum=WGS84")
msegsp.t <- spTransform(msegsp, CRSobj=lcc_proj4)
moresegdat$x <- msegsp.t@coords[, 1]
...
for(transect in unique(segdata$Transect.Label)){
  ...
  # select data from this transect for both segments and the Beaufort data
  ...
  # what is the closest Beaufort measurement?
